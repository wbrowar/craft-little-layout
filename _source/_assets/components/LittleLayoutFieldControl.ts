...
export class LittleLayoutFieldControl extends LitElement {
    ...
    private _getFieldValue() {
        if (typeof this._xStart === 'number' && typeof this._xEnd === 'number' && typeof this._yStart === 'number' && typeof this._yEnd === 'number') {
    ...
    private _getHasSelected() {
        ...
        return typeof this._xStart === 'number' || typeof this._xEnd === 'number' || typeof this._yStart === 'number' || typeof this._yEnd === 'number';
    }
    ...
    private _getSelectedBoxes() {
        ...
        if (this.selectionMode === 'box') {
            if (typeof this._xStart === 'number' && typeof this._yStart === 'number' && typeof this._xEnd === 'number' && typeof this._yEnd === 'number') {
    ...
    protected render() {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 0; c < cols; c++) {
                const column = c + 1;
                const row = r + 1;
                checkboxCols.push(html`
                    <button class="layout-box ${this._boxClassesForBox(column, row)}" type="button" @click="${ this.editable ? () => this._boxClicked([column, row]) : nothing }"></button>`)
            }
        ...
        return html`
            ...
            ${ import.meta.env.DEV ? debugInfo : nothing }
        `;
    }
    ...
    protected willUpdate(changedProperties: PropertyValues<this>) {
         ...
         *   _hasSelected
         */
        if (changedProperties.has('_xStart') || changedProperties.has('_xEnd') || changedProperties.has('_yStart') || changedProperties.has('_yEnd')) {
            this._fieldValue = this._getFieldValue();
         ...
         *   _selectedBoxes
         */
        if (changedProperties.has('_xStart') || changedProperties.has('_xEnd') || changedProperties.has('_yStart') || changedProperties.has('_yEnd') || changedProperties.has('selectionMode')) {
            this._selectedBoxes = this._getSelectedBoxes();
        }
    }
}